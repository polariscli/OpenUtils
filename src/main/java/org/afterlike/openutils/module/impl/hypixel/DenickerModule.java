...
public class DenickerModule extends Module {
	...
	private final Set<UUID> parsed = new HashSet<>();
	private static final Set<String> NICK_HASHES = new HashSet<>(
			Arrays.asList("4c7b0468044bfecacc43d00a3a69335a834b73937688292c20d3988cae58248d",
					...
					"cae2c0eb1730e11888e3f4dc133e6c5fd1434beb19b1616b026412fafc8e87",
	...
	private void parseSkinData(@NotNull NetworkPlayerInfo info) {
		...
		boolean denicked = false;
		String realName = null;
		// decode skin payload
		Property textures = texturesProperty(info.getGameProfile().getProperties());
		if (textures != null) {
			JsonObject root = decodeTexturePayload(textures.getValue());
			if (root != null) {
				// skin hash
	...
	private static @Nullable JsonObject decodeTexturePayload(final @Nullable String base64) {
		...
		try {
			...
			final String rawJson = new String(decoded, StandardCharsets.UTF_8);
			final JsonElement el = JsonParser.parseString(rawJson);
